                            ...
                            <div class="col-md-12">
                                <div class="row">
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <label>Order</label>
                                            <input type="number" name="Order" v-model="form.Order" class="form-control" :class="{ 'is-invalid': form.errors.has('Order') }">
                                            <div class="error" v-if="form.errors.has('Order')" v-html="form.errors.get('Order')" />
                                        </div>
                                    </div>
                                </div>
                            </div>
...
export default {
    ...
    methods: {
        getAllMenuItem(){
            this.isLoading = true;
            axios.get('/api/menu?page='+ this.pagination.current_page).then((response)=>{
        ...
        searchData(){
            axios.get("/api/search/menu/" + this.query + "?page=" + this.pagination.current_page)
                .then(response => {
                    this.menu_items = response.data.data;
                    this.pagination = response.data.meta;
                })
        ...
        getAllMenu(){
            axios.get('/api/get-all-menu').then((response)=>{
        ...
        update(){
            this.form.busy = true;
            this.form.put("/api/menu/" + this.form.Id).then(response => {
                $("#menuItemModal").modal("hide");
                this.getAllMenuItem();
            }).catch(e => {
                this.isLoading = false;
